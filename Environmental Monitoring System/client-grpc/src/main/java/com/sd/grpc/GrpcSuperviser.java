...
import com.sd.grpc.config.*; // Config gRPC
import com.sd.grpc.dto.*; // DTOs
...
/**
 * Supervisor que gerencia múltiplos simuladores gRPC.
 ...
 */
public class GrpcSuperviser {

    private static final Logger logger = LoggerFactory.getLogger(GrpcSuperviser.class);

    private final GrpcClientConfig config;
    //Armazena simuladores ativos por deviceId
    private final Map<String, GrpcSensorSimulator> simulators = new ConcurrentHashMap<>();
     ...
     * 
     * @param config configuração gRPC com URLs e intervalos
     */
    public GrpcSuperviser(GrpcClientConfig config) {
        this.config = config;
    ...
    public void start() {
        ...
        logger.info("gRPC supervisor started. Polling {} every {} seconds", 
    ...
    public void stop() {
        poller.shutdownNow();
        simulators.values().forEach(GrpcSensorSimulator::stop);
        simulators.clear();
    ...
    public void syncWithRegistry() {
        ...
        logger.info("Sync: found {} gRPC devices", devices.size());
        ...
        for (DeviceDTO device : devices) {
            if (device.isActive()) {
                ...
                // computeIfAbsent cria novo simulador se nao existe
                simulators.computeIfAbsent(deviceId, missingId -> {
                    logger.info("Starting gRPC device thread for {}", missingId);
                    GrpcSensorSimulator simulator = new GrpcSensorSimulator(
                        config.serverHost,
                        config.serverPort,
                        missingId,
        ...
        simulators.entrySet().removeIf(entry -> {
            if (!activeIds.contains(entry.getKey())) {
                logger.info("Stopping gRPC device thread for {} (removed or inactive)", entry.getKey());
                entry.getValue().stop();
                return true;
            }
            return false;
    ...
    /**
     * Obtém lista de dispositivos gRPC ativos do servidor de registro.
     * Realiza chamada HTTP GET para endpoint /api/devices/active/GRPC.
     */
    private List<DeviceDTO> fetchDevices() {
        try {
            String url = config.registryUrl + "/api/devices/active/GRPC";
